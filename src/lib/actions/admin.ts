...
/**
 * Auditor requests evidence that a ticked Action Plan item was really done
 * ("you marked the safeguarding policy renewed — upload it"). Flags the item on
 * the client's Action Plan and records the ask. Toggle off by passing note=''
 * is not supported here; use clearActionEvidenceRequest.
 */
export async function requestActionEvidence(
  input: z.infer<typeof requestEvidenceSchema>,
): Promise<ActionResult> {
  ...
  const { error } = await admin.from('action_states').upsert(
    {
      org_id: parsed.data.orgId,
      action_key: parsed.data.actionKey,
      evidence_requested: true,
      evidence_requested_at: new Date().toISOString(),
      evidence_note: parsed.data.note?.trim() || 'Please upload proof this was completed.',
    },
    { onConflict: 'org_id,action_key' },
  );
  if (error) return fail('Could not request evidence.');

  revalidatePath('/admin/customers');
  return { ok: true };
}

/** Clear an evidence request once the auditor is satisfied. */
export async function clearActionEvidenceRequest(orgId: string, actionKey: string): Promise<ActionResult> {
  await requireAdminSession();
  const admin = createAdminClient();
  const { error } = await admin
    .from('action_states')
    .update({ evidence_requested: false, evidence_note: null })
    .eq('org_id', orgId)
    .eq('action_key', actionKey);
  if (error) return fail('Could not clear the request.');
  revalidatePath('/admin/customers');
  return { ok: true };
}
...
export async function generateReport(auditId: string): Promise<ActionResult> {
  await requireAdminSession();
  return generateAuditReport(auditId);
}
